...
export default function AnadirUsuario() {
    ...
    const handleClick=(e)=>{
      ...
      if(nombre.replace(/ /g,'')=== '' || correo.replace(/ /g,'') === '' || rol.replace(/ /g,'') === '') {
        setMensaje('Error en la petición!');
      }

      else {
        fetch("http://localhost:8082/epac/usuario/save", 
          {
          method:"POST",
        ...
        .then((result)=>{
          setUsuario(result);
          setPetHecha(true);
          setMensaje('Usuario Añadido');
          console.log("Nuevo usuario anadido");
          setNombre('');
          setCorreo('');
          setRol('');
        })
    ...
    return (
      <Container>
        <Paper elevation={3} style={paperStyle}>
          <h1 style={{color:"rgb(77, 77, 255)"}}>Añadir Usuario</h1>
        <Box
          component="form"
          sx={{
            display: 'flex',
